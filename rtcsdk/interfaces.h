...
template<typename Derived, typename FirstInterface, typename... OtherInterfaces>
HRESULT object<Derived, FirstInterface, OtherInterfaces...>::factory_create_object(const GUID &iid,
                                                                                   void **ppv,
    ...
    try {
        ...
        if (pOuterUnk == nullptr) {
            if constexpr (check_trait<trait_singleton_factory>()) {
                ...
                if constexpr (check_trait<trait_smart_singleton_factory>()) {
            ...
            if constexpr (check_trait<trait_supports_aggregation>()) {
                ...
                return CLASS_E_NOAGGREGATION;// NOLINT (clang-diagnostic-unreachable-code)
            }
        }
        return hr;
